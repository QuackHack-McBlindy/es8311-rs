...
impl Es8311 {
    /// Default I2C address for ES8311 (often 0x10 or 0x11).
    pub const DEFAULT_ADDR: u8 = 0x10;
        ...
        D: DelayNs,
    {
        ...
        delay.delay_ns(20_000_000); // 20 ms
    ...
    /// Dump all registers to console (requires `defmt` feature).
    #[cfg(feature = "defmt")]
    pub fn register_dump<I2C, E>(&self, i2c: &mut I2C)
    where
        I2C: I2c<Error = E>,
        E: core::fmt::Debug,
    {
        for reg in 0..0x4A {
            match self.read_reg(i2c, reg) {
                Ok(val) => defmt::info!("REG:{:02x}: {:02x}", reg, val),
                Err(_) => defmt::error!("Failed to read REG:{:02x}", reg),
            }
        }
    }
}
...
// -----------------------------------------------------------------------------
// Clock coefficient table (identical to original)
// -----------------------------------------------------------------------------
#[derive(Clone, Copy)]
struct CoeffDiv {
    mclk: u32,
